...
\todo{merge the following descriptive list with the following paragraph}
\begin{itemize}
\item In the first step (cf. \ding{182}) the \B primitive is activated.
...
\item On the first activation there is no native code available yet.
	Hence the primitive will fail and the \ST body (cf. \ding{183}) of the \B method gets evaluated.
	This is where \B prepares the native code for the \FFI callout.
...
\item Again we end up in the \B activation primitive (cf. \ding{185}).
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=1.1]{benzo-debugger}
...
The \textsc{Intel x86} instruction set is widely distributed among a variety of operating systems and thus the primary choice for \B.
Since the generated code is independent of C functions it will work on all platforms with \x86 support.
However, other architectures such as \ARM start to be more widespread on portable devices.
...
While there is no way around a \ARM version of the underlying assembler, it does not pay off to port all the existing \B routines to each new platform.
Instead we suggest to use an intermediate format that is platform independent with the following properties:
\begin{itemize}[noitemsep]
...
\paragraph{\VCPU.}
Along with the ongoing work for this thesis we started \VCPU, a first attempt for such an implementation.
\VCPU is based on an \TAC to simplify the adoption of optimizations such as \SSA.
A \TAC instruction takes the following form.
%
\begin{stcode}{}
...
\end{stcode}
The same functionality expressed in \x86 instructions is as follows:
\begin{stcode}{}
Benzo x86 generate: [ :asm |
...
\cb{messy here, too tired: create proper link between the two sections}
\noindent Even though the \VCPU has a built-in register allocator and a minor optimization pipeline it yields the following inferior instructions:
\begin{stcode}{}
mov   6 -> EDX
mov   2 -> ECX
add EDX, ECX
