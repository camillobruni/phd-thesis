...
Language-side libraries, reflective capabilities, VM extensions or hybrid approaches are other possibilities which we discuss in detail in \secref{related}.
...
It relies only on a small set of generic VM extensions described in \secref{vm-requirements}, whereas the vast majority of the framework is implemented as a language-side library.

\subsection{\B Applications.}
In \secref{use case} we advocate the contribution of this approach by providing three different incremental examples that heavily use the framework.
...
\begin{description}
	\item[FFI] A complete language-side Foreign Function Interface (FFI) implementation, described in \secref{ffi}.
	\item[Dynamic Primitives] A language-side compilation toolchain that replaces system primitives at runtime with customized code, described in \secref{waterfall}. 
	\item[Language-side JIT Compiler] A JIT compiler that works at language-side and interacts with the VM for code synchronization, described in \secref{nabujito}
\end{description}
...
\figref{smalltalkPrimitive}-a describes the case where the primitive is successfully executed.
...
\figref{smalltalkPrimitive}-b describes it. 
...
\figref{nativeCodeMethod} shows the structure of a \ST compiled method that has native code attached to it.
...
This means that for native code, instead of a method-local literal array we share a global literal array as shown in \figref{externalRoots}. 
...
The performance boost of this optimization is further discussed in \secref{issues-performance}.
...
Additional methods cover the access to the external roots described in \secref{gc-interaction}.
...
In section \secref{use case} we introduce more substantial applications based on \B.
...
\end{enumerate}
%
The example in \lstref{basic-native-code} uses \B's generator to create and install the native code which would trigger a low-level interrupt. Behind the scenes \B adds some more information to the code as the already mentioned platform marker. 
...
This mechanism is shown in \figref{nativeCodeActivation}.
...
%\del{\figref{ffi} shows how only the last step in calling an external function relies on low-level VM interaction.}
%\secref{ffi-low-level} explains the execution component details.
%Via reflection techniques \NB provides a simple yet powerful marshalling library which is further described in \secref{ffi-symbiosis}.
%
%----------------------------------------------------------------------------
...
In particular \NB, the \B-based FFI presented in \secref{ffi}, is used for interfacing with C libraries (\ttt{dlsym}). 

\paragraph{Primitives in Smalltalk.}
As already partially explained, whenever a method is compiled with the \ttt{primitive} pragma as shown in \secref{vm-interaction} a flag is set on the \ttt{CompiledMethod}. 
...
Waterfall uses \B's mechanism for replacing primitive methods with customized versions that are nativized dynamically as described in \secref{benzo}.
...
VM extensions are only justified in the presence of strong performance requirements (see \secref{related}).
...
This guarantees that the VM stays as long as possible in the JIT context (see \secref{jit-interaction}). Additionally this enables new performance optimizations that for instance are hard to achieve with standard compliant C code.
A typical example is the integer addition which has to deal with overflow checks and conversion of tagged integers.
In \secref{waterfall} we describe how Waterfall suffers a similar constraint. Waterfall manually defines such primitives in terms of native assembler instructions through the language-side \B interface.
...
In \secref{benzo} we explained how \B causes only a one-time overhead for native code generation. 
Thereafter it is cached for later activations.
The three use case presented in \secref{use case} heavily benefit from this fact.
...
Even better, for instance the \B-based FFI implementation presented in \secref{ffi} outperforms a VM-level FFI-plugin due to a more flexible language-side implementation. 
These results are shown in the following \tabref{ffi-performance-simple}.
...
Compared to a static plugin-based FFI implementation \NB has only a one-time startup overhead with its numbers shown in \secref{issues-performance}.
...
\tabref{ffi-performance-simple} measures the accumulative time of 100'000 FFI calls.
...
The JIT interaction described in \secref{jit-interaction} is also an important optimization factor especially when calling out small helper routines where the context switch from jitted mode is not negligible.
...
From the in-production experience of NativeBoost, the \B-based FFI (see \secref{nb-performance}), we know that these costs amortized, especially for long-term applications.
...
In \tabref{nabujito-performance-small} we compare the compilation speed of the standard \PH compiler and Nabujito.
...
We see that in the simple case displayed in \tabref{nabujito-performance-small} Nabujito's compilation speed lies within the same range as the standard \ST compiler.
We expect that in the future we apply more low-level optimizations and thus increase the compilation time of Nabujito.
However, we have shown in the performance evaluation for \NB, the \B-based FFI, in \secref{nb-performance} that even a rather high one-time overhead is quickly amortized.
...
On a more abstract scale \B allows for a new way of extending the complete language runtime, hence we classify the related work according the following categories show in \figref{extensionComparison}: general language-side extensions, extensions using reflection, VM-level extensions, and hybrid approaches.
%
...
%The relation between the side concerning the abstraction and implementation levels (VM vs. Language) of these extensions is illustrated in \figref{extensionComparison}.
...
Compared to \ugh{our} approach the efficient VM implementation suffers from the shortcomings described in \secref{vm-extensions}. 
