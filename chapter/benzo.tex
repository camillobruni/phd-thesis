...
\B provides a unique experience of being low-level yet using high-level concepts at the same time.
...
% ---------------------------------------------------------------------------
\subsection{\B a Framework for Reflective High-level Low-level Programming}
% ---------------------------------------------------------------------------
...
\B originally evolved from the work done by Igor Stasenko on the \NB which we will describe in more detail in the following \chapref{FFI}.
Initially \B itself was not considered as a separate application but a core of the \FFI framework.
We extracted the core concepts –- dynamic high-level low-level programming -- into a separated framework, \B.
...
\begin{figure}[ht]
	...
	\caption[\PH Primitive]{Generic primitive methods in \PH: a) By default a primitive completely bypasses the bytecode, b) A failing primitive executes the bytecode as fallback.}
...
\begin{figure}[ht]
    ...
    \caption[\ttt{CompiledMethod} With \B Code]{
    Example of \PH method with native code which calls the low-level debug exception handler \ttt{INT3}. 
    The bytecode references objects such as symbols for messages sends indirectly via the literal frame residing at the beginning of the method.}
...
\begin{figure}[ht]
	...
	\caption[\B Native Code Acivation]{
	Native code activation with \B: The first call triggers the code generation written in \PH.
	Then the method is restarted and the cached native code executed.}
...
\item[Code Installation:] After installing the native code in the method trailer, the \B-enabled method is reactivated with the original arguments (cf. \ding{184}).
	For activation \B uses \PH's \ttt{\#perform:} \ttt{withArguments:} to reflectively restart the method.
...
\begin{figure}[h]
	...
	\caption[\NB Overview]{
	\NB generates native code at language-side with \B to perform an \FFI callout to an external function.}
...
\noindent \WF is the second application we use to validate \B.
This project has been developed by Guido Chari and allows a programmer to dynamically replace primitives at language-side, essentially reflectively altering the \VM.
This is a step further than previously presented \B-based \FFI which allows us to call external functions by generating the callout code at language-side.
...
As an example performance evaluation we present is for \NB the \B-based \FFI.
...
The following table contains a performance comparison of three different \FFI implementations for \PH that represents the typical use case.
...
The existing language-side C-\FFI has a generic plugin to call C-functions and performs type-conversions at language-side.
...
The plugin-based C-\FFI is also slower than \NB since it still has generic conversion function for \PH objects, albeit written in C and thus faster than in \Alien.
...
\begin{stcode}[caption={Simple \B code possibly leading to an unbalanced stack.}]{3}
Benzo x86 generate: [ :asm |
...
\end{stcode}
%
The context is exposed as first-class object to the language and thus we can reflectively modify it.
However, at the same time there it has to fulfill a contract with the \JIT which maps it to a native stack frame for performance reasons.
In the current \Cog \VM reflective accesses to the current context/stack frame are not checked.
Hence we can use it to inject a wrong object, in our case a small integer.
This would require additional checks at the \JIT to properly read the small integer.
However, for performance reasons this is not done, resulting in a wrongly read value and a subsequent crash of the \VM.
...
Implementing a debugging version of the \B plugin to activate native code in a forked process would solve this issue, though at the cost of an additional primitive and the use of an old \VM.
\lt{Why doesn't Pharo support forking}
...
\begin{figure}[h]
	...
	Outline of a low-level debugging mechanism to transform low-level errors into a high-level exception using a designated signal handler which walks the stack until it finds the \ttt{primitiveNativeCall} activation.}
	\figlabel{benzo-debugger}
\end{figure}

\noindent The following list explains the detailed steps of \figref{benzo-debugger}.
\begin{enumerate}
...
Nevertheless, this scheme could bring approximate a high-level user experience for native \B code.
...
\end{itemize}
%
Applying these compiler passes to the example in \lstref{benzo-problem-vcpu} yields the following native instructions with explicit register assignments:
%
\begin{stcode}{}
mov   6 -> EDX
mov   2 -> ECX
...
\lt{Although Section 3 does mention in one place that Benzo isn’t created by the thesis author, it’s easily missed, and much of the section 3 unintentionally gives the impression that Benzo is a contribution of this thesis. For example, the first section of Section 3.7: “In this chapter we presented Benzo” should probably be something along the lines of “In this chapter, I gave an overview of Benzo, a pre- existing compilation system embedded in Pharo, its strengths and weaknesses” (not necessarily that, but hopefully one gets the idea).}
