...
The current \VM uses a moving Garbage Collector (\GC) with two generations \sm{ref?} and uses a \JIT that applies basic register allocation to reduce stack load. 
...
\sm{Really? This ain't a blog post...}
...
\sm{Avoid implicit and indirect statements, it is not 'a' method, it is the multiplication method}
%
...
The interaction of \B and the JIT.
...
\del{For pragmatic reasons current implementation only supports \textsc{x86} and \textsc{x86-64}.}
...
Motivated by the aforementioned implications of a \VM-level \JIT we conceived \B a prototype \JIT compiler based on \B.
...
\sm{Specific vs. specialised, also, not clear enough what that really means here }
...
\sm{Slower, why? And why generally? I believe proper language side JITs will optimize themselves, and be as fast, or faster than naive C implementations}
