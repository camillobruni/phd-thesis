...
In this first part of the \WF validation we compare the performance of primitives in \PH.
Once for the simple integer operation "greater than" (\ttt{$>$}) and once for the essential primitive \ttt{basicNew} which we already showed in \lstref{validation-basicnew}.
...
\begin{enumerate}[noitemsep,nolistsep]
	\item The first approach is the implementation with \WF.
	\item For the second approach we run the fallback language-side implementation that is triggered whenever the standard primitive failed.
	\item Finally we the fast standard primitive provided by the \VM.
\end{enumerate}
%
