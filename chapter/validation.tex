...
\WF in the core is a \Slang compiler implemented purely in \PH relying on the existing \B infrastructure. 
...
\noindent In this section we present \NBJ, a \B-based approach for a language-side \JIT compiler that tries to replicated the behavior of the existing \JIT as shown in \figref{validation-nabujito-overview}.
...
\begin{figure}[h]
	...
	\figlabel{validation-nabujito-overview}
\end{figure}
...
Which means that recursion in a \PH program is not a direct danger.
...
However, inlining is a difficult task in a highly dynamic language such as \ST or \Self \cite{Cham89a} when using a per method \JIT.
A better match for this seems to a trace-based \JIT such as the one present in \PyPy or \Luajit.
