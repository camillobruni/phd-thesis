...
We only chose to implement a small subset of them with \B and reuse the existing routines for simplicity.
...
However, to properly interact with the \JIT we have to provide certain low-level meta-information.
Namely we have to tell the \JIT where references to language-side objects are located in the native code.
