...
\paragraph{Primitive Instrumentation}
...
Then this \AST representation is translated to native code enforcing C-like \Slang semantics.
...
\begin{itemize}[noitemsep]
	\item Installed \Slang sources
	\item \B-based compilation toolchain
\end{itemize}
...
\todo{\Slang to \AST}\\
\todo{\AST to \IR}\\
\todo{\IR cleanup transformation}\\
\todo{\IR to native code compilation}

 The current prototype has only three fully implemented stages: \Slang to \AST, \AST to an \IR (between \TAC and \SSA) and finally \AST or \IR to native.
...
\todo{Explaing step by step the compilation and installation proceedure for dynamic primitives}

% -----------------------------------------------------------------------------
\paragraph{Dynamically Replacing Plugins}
\todo{explain step by step the proceedure to dynamically install new plugins}
