...
\cd{Je souhaite grouper dans ce rapport les chapitres 5 et 6}
\cd{Les descriptions de ces deux systemes sont tres techniques et parfois cryptiques}
...
\sm{I would sell and phrase that differently. The prototype is the standard thing, you aren't getting any less points just because it is a prototype. Instead, you might have points in your favour for the fact that nativeboost is stable enough for use in production.

So, instead of presenting the other ones as 'less worthy' present NB as more awesome...}

As first we will present \WF \todo{by Guido Chari}, dynamic primitives based on \B.
...
\sm{To tears, don't follow, to many points discussed.
High level intuition and distinction between approaches, (and remind me that I read something about it before...)}
...
\seclabel{validation-waterfall}
% ===========================================================================

\lt{I found the description of Waterfall a bit confusing. The motivation is OK, but the performance of Waterfall is sufficently poor that I don’t think anyone would want to use it yet. This raises the obvious question: could such a mechanism ever be sufficiently performant? This isn’t tackled, other than to say performance issues need to be looked at. This is unfortunate, because without knowing if Waterfall could be made performant, it’s difficult to know if this is an interesting curio, or a viable approach. Perhaps making it clear upfront that this is an inconclusive experiment might help the reader?}
...
\sm{Much of that as discussed before... At least link back}
...
\gt{I have, however, not understood all the process, especially, the figure 5.2. At one point in the figure, an AST is converted in assembly, but I don’t understand which component is in charge of the conversion and if a full compiler is written for that purpose?}
...
\begin{table}[H]
    ...
    \begin{tabular}{rSS}
		...
		Fallback	& 195.0(32)           & \approx30.0 \\
		\WF	        &  22.8(34)           & \approx3.6
    \end{tabular}
...
\begin{table*}[h]
    ...
    \begin{tabular}{rSS}
        ...
        Secure reflective instrumentation & 27.72(80)           & \approx 99 \\
        \WF-based instrumentation         &  7.72(54)           & \approx 28 \\
        \WF-based non-instrumentation     &  7.08(46)           & \approx 25 \\
    \end{tabular}
...
\sm{Of course it is a prototype, that's not a problem
And it is not problems, but limitations}
...
\sm{This is not clear, because the distinction is not clear to me here: back reference? Where did you make it clear?

Also, the paragraph is again reversed to what I would expect, start also with something positive.}
...
\lt{I found the description of Nabujito very confusing. The basic premise is simple: 3
a JIT for Pharo written in Pharo. However, Section 5.2 spends almost all of its time explaining why Nabujito doesn’t really work. This meant that I ended up a bit unsure which bits did work. The sudden suggestion of a different JIT (Sista) doesn’t really include any hints of whether it was influenced by Nabujito or not.}

\lt{Put another way: why should the reader care about Nabujito? At the moment, the way it’s presented makes it sound like a dead-end experiment from which few lessons are learnt. Those are rarely interesting, and I think a little bit of work needs to be put in to motivate the reader.}


\noindent In this section we present \NBJ, a \B-based approach for a language-side \JIT compiler that tries to replicated the behavior of the existing \JIT.

\begin{figure}[h]
	\centering
	\includegraphics[scale=\imagescale]{nabujito-overview}
	\caption[\NBJ Overview]{
	\NBJ tries to replicate the \JIT which resides in the \VM.
	We use \B to compile native versions of \PH methods that match the results generated from the existing \JIT.}
\end{figure}

We have presented two other \B applications so far: a \FFI and previously the dynamic primitives.
Both of these applications have in common that the switch or call to native code happens at the end of the \PH stack as shown in \figref{stack-waterfall}.
%
\begin{figure}[h]
	\centering
	\includegraphics[scale=\imagescale]{stack-waterfall}
	\caption[\WF Stack Usage]{
	\WF and \NB use native code at the end of the \PH stack.}
	\figlabel{stack-waterfall}
\end{figure}
%
...
\figref{nbj-stack-usage} shows how the \B-based native code is embedded into the \PH stack.
%
\begin{figure}[h]
	\centering
	\includegraphics[scale=\imagescale]{stack-nabujito}
...
\sm{GC is not the problem, because contexts are recycled 

Without the JIT using the C stack gives. 30\% or so, if I remember correctly. There is some GC benefit in it, but I guess mostly locality and more efficient encoding. 

For the JIT it is much better because you can encode the stack much more in line with normal native code.

I think... Should be checked, but it is not just the GC}
...
\sm{Why is all of this important? Where are you going? Tell me!}
...
Which means that recursion in a \PH program is not a \ugh{direct threat}.
...
\begin{itemize}
	...
	\sm{That's the problems you are setting out to solve}
\end{itemize}
...
\sm{Or when the inlined method is polymorphic you are starting to hit another code path....}
