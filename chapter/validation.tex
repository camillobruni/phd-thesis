...
\todo{lanugage has infinite stack, while the bytecode interpreter uses a loop} \\
\todo{\JIT uses native stack, so does the bytecode interpreter} \\
\todo{returning inside native code should also return on the native stack} \\
\todo{switching to native code would not increase the C stack, while calling the bytecode interpreter from the \JIT code would} \\
\todo{avoid ever-growing C stack}
...
\todo{common experiment to reuse single primitive definition}
