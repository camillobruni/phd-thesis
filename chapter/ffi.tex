...
% ===========================================================================
\sm{Background should go into chapter 2}
...
\begin{description}
	...
	\sm{Why does it need more than benzo, why is it not presented as building on benzo, why is it presented again at all?}
...
\sm{I think you should postpone the whole nativeboost stuff until here, and then do it properly}
...
\lt{Why is the abs call annotated but not clock? It feels odd that parts of abs are explained in detail after they’ve already been seen in clock (e.g. primi- tiveNativeCall). Frankly, I think the clock example could be removed; abs is still simple enough to be explained in one go.}
...
\lt{I’d like to know how NativeBoost handles C structures, particularly the issue of alignment, which is critical to interacting with C. Does NativeBoost encode the rules of a given C compiler? Does it call out to a C compiler to ensure things are done correctly? or...?}
...
\sm{I am not reading this, you didn't tell me why I should care about all those details

Ok, you implemented that stuff, cool, but he concrete nativeboost design seems to be only vaguely related to your thesis goal.

This is something you'll need to discuss with your thesis promotor, but, for me, this does not belong into this text

You can leave it in, but I would suggest to make clear with headings and section naming that I can safely skip all of it, without missing any essential thesis related discussions }
...
\begin{figure*}[h]
	\begin{adjustwidth}{-2.0in}{-2.0in}
		\centering
		\includegraphics[width=0.9\paperwidth]{ffiOverview}
	\end{adjustwidth}
...
\sm{I don't like the split into multiple benchmark sections... All evaluation belongs into this chapter}
...
From the 100 probes we show the average and the standard deviation for a $95\%$ confidence interval in a gaussian distribution.
\sm{First, how do you know the results are normally distributed?
Second, 68\% really, why? That will raise a lot of red flags!!!!!}
...
\sm{What about the noise you get from measuring on a end user used machine? I would worry more about that than anything else}
...
\sm{Clock? Really? Are you sure that one doesn't do any kind of kernel calls, introduces scheduling points or what ever the OS is feeling like?}
...
\begin{table}[H]
    ...
    \begin{tabular}{rSS}
        ...
        \NB          & 492.1 (14)       & 1.0 \\
        \Alien       & 606.6 (48)       & \approx 1.2\\
        C-\FFI       & 541.7 (18)       & \approx 1.1\\
        \LuaJIT      & 343.0 (24)       & \approx 0.7
    \end{tabular}
...
\sm{Same complexity, really? Most of the time it is not even a function, and it certainly does not involve a system call.

Look at the numbers, it doesn't look like being the same to me...}
%
\begin{table}[h!]
    \centering
    \begin{tabular}{rSS}
                & {Call Time [ms]} & {Relative Time} \\\midrule
        \NB     &  65.34(46)       &         1.00 \\
        \Alien  & 175.77(62)       & \approx 2.69 \\
        C-\FFI  & 148.77(42)       & \approx 2.27 \\
...
Strictly speaking we do not measure the pure marshalling overhead, but we chose methods that heavily depend on string arguments to be converted between the internal type in \PH and the external type expected by the C function.
%
\begin{table}[h!]
    \centering
    \begin{tabular}{rSS}
                    & {Call Time [ms]} & {Relative Time} \\\midrule
        \NB         &  105.29(48)      &          1.0 \\
        \Alien      & 1058.70 (400)      & \approx 10.1 \\
        C-\FFI      &  282.94(48)      & \approx  2.7 \\
        \LuaJIT\tablefootnote{Downsampled from increased loop size by a factor $10$ to guarantee accuracy.\sm{You assume that runtime increases linear, is that reasonable? What about nondeternemism in memory management, etc?}}
                    & 97.00     (100)         & \approx 0.9
    \end{tabular}
...
The marshalling overhead is less than for the previous \ttt{getenv} example as the strings passed and returned are smaller.
...
\begin{table}[h!]
    ...
    \begin{tabular}{rSS}
                    & {Call Time [ms]}  & {Relative Time} \\\midrule
        \NB         &  371.03(102)       &         1.0 \\
        \Alien      & 1412.37(158)       & \approx 3.8 \\
        C-\FFI      &  605.02(46)       & \approx 1.6 \\
        \LuaJIT     &  202.40 (420)       & \approx 0.6
    \end{tabular}
...
\begin{table}[h]
    ...
    \begin{tabular}{rSS}
                    & {Call Time [ms]} & {Relative Time} \\\midrule
        \NB         &  79.00  (54)      &         1.0 \\
        \Alien      & 753.82 (102)      & \approx 9.5 \\
        C-\FFI      & 380.80 (540)      & \approx 3.6 \\
        \LuaJIT     &   5.66  (30)      & \approx 0.07
    \end{tabular}
...
\begin{table}[h]
    ...
    \begin{tabular}{rSS}
                    & {Call Time [ms]} & {Relative Time} \\\midrule
        \NB         & 2300.00 (220)      &         1.0 \\
        \Alien      &  600.83 (70)     & \approx 0.26 \\
        C-\FFI      & NA               & NA \\
        \LuaJIT     &   46.13(124)     & \approx 0.02\\ \cmidrule(r){2-3}
	\NB with Native Callbacks
	                &    4.98(42)      & \approx 0.002
    \end{tabular}
...
% ===========================================================================
\sm{Why does that come here, after evaluation?}
...
As explained in more detail in \secref{benzo-gc-interaction} \B uses a special array at a known fixed address to deal with this problem.
...
\seclabel{ffi-problems-platform}
% ---------------------------------------------------------------------------
\sm{What does that have to do with your thesis?}
...
For instances the \VM has a separate process running for handling input events, which does not get forked automatically.
Additionally we see certain file handles that are not properly recreated for the child process.
...
The \OS environment implementation based on \NB described earlier was such an attempt.\todo{check where mentioned}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{nb-deployment-mode}
...
With a partial solution ready to integrate \NB closer with the \JIT we expect to come closer to \Lua's performance.
We implemented already a \JIT enabled \B primitive that inlines the language-side native code into the jitted code and thus avoids the overhead of the activating a primitive each time.
However, this requires certain changes when generating native code in \NB.
Namely, we have to respect the register strategy used by the \JIT.
Due to these non-trivial changes we have not ported \NB to the \JIT-enabled \B primitive yet.
