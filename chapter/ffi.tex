...
This paper presents \NBFFI\footnote{\url{http://code.google.com/p/nativeboost}} an FFI library at language-side for Pharo that supports callouts and callbacks, which we present in \secref{nutshell}.
...
\lstref{clock} shows the code of a regular Smalltalk method named \ttt{ticksSinceStart} that defines a callout to the \ttt{clock} function of the \texttt{libc}.
...
\lstref{AthensCairoSurface} shows how to write a regular Smalltalk class to wrap a C structure.
...
\tabref{wrappingstruct} shows a comparison between C-managed and Pharo-managed structures.
...
\paragraph{Evaluation.}
In \tabref{performance-structs} shows that \NB outperforms the two other Smalltalk implementations.
...
\figref{externalRoots} visualizes how native code directly accesses Pharo objects through this indirection.
...
The examples in \secref{nutshell}, especially \figref{nativeBoostSyntax} show that each \NB FFI callout requires a special primitive.
