...
\lstref{clock} shows the code of a regular \ST method named \ttt{ticksSinceStart} that defines a callout to the \ttt{clock} function of the \ttt{libc}.
...
\lstref{getenv} shows a callout declaration to the \ttt{getenv} function that takes one parameter.
%
\begin{stcode}[
	label={lst:getenv},
...
\end{table}
\lstref{setenv} shows another example to callout the \ttt{setenv} function.
...
\lstref{calloutOpaqueStruct} shows a callout definition to the \ttt{cairo\_image\_surface\_create} function that returns a \ttt{cairo\_surface\_t*} data type.
...
Conversely, passing an \ttt{AthensCairoSurface} object as a parameter in a callout makes its pointer stored in its \ttt{handle} iv (cf. \lstref{calloutOpaqueStructParameter}) to be passed.
...
\lstref{cairo_c_definition} shows an example of a C struct type definition for \ttt{cairo\_matrix\_t}.
...
\noindent \lstref{AthensCairoMatrix} shows that the \ttt{NBExternalStructure} of \NBFFI can be subclassed to define new types such as \ttt{AthensCairoMatrix}.
...
\noindent \lstref{cairoCallouts} shows a callout definition to the
