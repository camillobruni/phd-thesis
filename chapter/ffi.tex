...
\figref{extensionComparison} depicts four possibilities of dealing with new or external libraries in a high-level language.

\paragraph{Language-side Library.}
One solution is to re-implement a library completely at language-side (cf. \figref{extensionComparison}.a).
...
\paragraph{FFI.}
A higher-level solution is to define \emph{Foreign Function Interfaces} (FFIs) (cf. \figref{extensionComparison}.d).
...
Both these pitfalls are presented in more detail in \secref{evaluation}.
...
% Alien and C-FFI run in the same \PH image as NativeBoost allowing a much closer comparison.
...
This paper presents \NBFFI\footnote{\url{http://code.google.com/p/nativeboost}} an \FFI library at language-side for \PH that supports callouts and callbacks, which we present in \secref{nutshell}.
...
C-\sFFI is fast because it is mostly implemented at VM-level, however it is limited when it comes to do complex calls that involve non-primitive types or when we want to define new data types.
...
%We then compare performance of \NBFFI with three other \FFI implementations in \secref{evaluation}.
%The following \secref{internals} gives more insights on the \NB.
%After the related work presented in \secref{relatedWork} and we conclude this paper in \secref{conclusion}.
...
\lstref{clock} shows the code of a regular \ST method named \ttt{ticksSinceStart} that defines a callout to the \ttt{clock} function of the \texttt{libc}.
...
\figref{nativeBoostSyntax} presents the general syntax of \NBFFI through an example of a callout to the \ttt{abs} function of the \texttt{libc}.
...
An \texttt{errorCode} is returned by this primitive if it fails and the regular \ST code below is executed (cf. \ding{184}).
...
\lstref{getenv} shows a callout declaration to the \texttt{getenv} function that takes one parameter.
...
\tabref{nbPrimitiveTypes} shows the default and automatic data conversions achieved by \NBFFI.
...
\end{table}
\lstref{setenv} shows another example to callout the \texttt{setenv} function.
The return value will be converted to a \ST \texttt{Boolean}.
The two first parameters are specified as \texttt{String} and will be automatically transformed in \ttt{char*} with an ending \cnull character.
The last parameter is \texttt{1}, a \ST literal value without any type specification and \NB translates it as an \ttt{int} by default.
...
Another interesting example of automatic marshalling is to define the \ttt{abs} method (cf. \figref{nativeBoostSyntax}) in the \ttt{SmallInteger} class and passing \texttt{self} as argument in the callout.
In such case, \NB automatically converts \texttt{self} (which is a \ttt{SmallInteger}) into an \texttt{int}.
This list of mapping is not exhaustive and \NB also supports the definition of new data types and new conversions into more complex C types such as structures (cf. \secref{internals}).
...
\lstref{AthensCairoSurface} shows how to write a regular \ST class to wrap a C structure.
...
\lstref{calloutOpaqueStruct} shows a callout definition to the \texttt{cairo\_image\_surface\_create} function that returns a \ttt{cairo\_surface\_t*} data type.
...
Conversely, passing an \texttt{AthensCairoSurface} object as a parameter in a callout makes its pointer stored in its \texttt{handle} iv (cf. \lstref{calloutOpaqueStructParameter}) to be passed.
...
\lstref{cairo_c_definition} shows an example of a C struct type definition for \texttt{cairo\_matrix\_t}.
...
\lstref{AthensCairoMatrix} shows that the \texttt{NBExternalStructure} of \NBFFI can be subclassed to define new types such as \texttt{AthensCairoMatrix}.
...
\lstref{cairoCallouts} shows a callout definition to the
...
\tabref{wrappingstruct} shows a comparison between C-managed and \PH-managed structures.
...
\end{stcode}
%
Code \lstref{calloutWithCallback} shows the primary \PH method for invoking \ttt{qsort} with a \ttt{QSortCallback} instance for the compare function.
...
\figref{nbArchitecture} describes the general architecture of \NB.
...
\figref{ffi} shows a comparison of the resolution of a \FFI call both in \NBFFI and a plugin-based \FFI.
...
\begin{table}[h!]
    ...
    \end{tabular}
    \caption{Speed comparison of an \ttt{int abs(int i)} \FFI call (see \figref{nativeBoostSyntax}).}
...
In \tabref{performance-abs} we see that \NB is at least a factor two faster than the other \ST implementation.
...
\paragraph{Evaluation.}
\tabref{performance-clock} and \tabref{performance-abs} call C functions that return integers for which the conversion overhead is comparably low.
However we see that Alien compares worse in the case of more complex Strings.
\tabref{performance-getenv} and \tabref{performance-printf} show this behavior.
...
\tabref{performance-structs} shows the results.
...
\begin{table}[h!]
    ...
    \end{tabular}
    \caption{Speed comparison of an \ttt{cairo\_matrix\_multiply} \FFI call (cf. \lstref{cairoCallouts})}
 	\tablabel{performance-structs}
\end{table}

\paragraph{Evaluation.}
In \tabref{performance-structs} shows that \NB outperforms the two other \ST implementations.

% ===========================================================================
\subsection{Callbacks}

\tabref{performance-qsort} shows a comparison of \texttt{qsort} callouts passing callbacks.
...
\begin{table}[H]
    ...
    \end{tabular}
    \caption{Speed comparison of a \ttt{qsort} \FFI call (cf. \lstref{calloutWithCallback})}
...
To really demonstrate that these context switches are the bottleneck, \tabref{performance-qsort} also shows the result of doing the same benchmark in \NB but using a native callback i.e. containing native code.
...
The various examples shown in \secref{nutshell} show how an \FFI callout is defined in a standard method.
...
\NB then reflectively resends the original message with the original arguments (for instance \ttt{abs:} in the example \figref{nativeBoostSyntax}).
On the second activation, the native code is present and thus the primitive will no fail but run the native code.
\secref{nbPrimitive} will give more internal details about the code activation and triggering of code generation.

% ---------------------------------------------------------------------------
\subsubsection{Generating Assembler Instructions}

\figref{nbArchitecture} shows that \NB relies on AsmJit\footnote{\url{http://smalltalkhub.com/\#!/~Pharo/AsmJit}}, a language-side assembler.
...
As shown in the examples in \secref{nutshell} and in more detail in \figref{nativeBoostSyntax} \NB detects which method arguments correspond to which argument in the \FFI callout.
...
\begin{figure}[h]
	...
	\caption[\NBFFI External Roots]{Pointers in a \ttt{CompiledMethod} to objects registered as external roots are pinpointed at fixed offset in global VM-level object.}
...
\figref{externalRoots} visualizes how native code directly accesses \PH objects through this indirection.
...
In \secref{generating} we explained how \NB creates \FFI callouts at language-side.
However, so far we left out the part on how the generated native code is activated.

The examples in \secref{nutshell}, especially \figref{nativeBoostSyntax} show that each \NBFFI callout requires a special primitive.
\figref{nativeCodeActivation} shows how a \NB method is activated.


\begin{figure}[h]
	\centering
	\includegraphics[scale=1.1]{nativeCodeActivation}
	\caption[\NB Native Code Activation]{\todo{Add Primitive Failure Arrow}Native code activation. The first call triggers the code generation. Then the method is restarted and the native code executed.}
...
Using a in depth evaluation of \NB comparing against two other \ST \FFI implementations and \LuaJIT we showed in \secref{evaluation} that our language-side approach is competitive.
