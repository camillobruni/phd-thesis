...
\todo{say that from now one by \emph{reflection} or \emph{dynamic reflection} we usually imply intercession + structural AND behavioral reflection}

% ---------------------------------------------------------------------------
\subsection{Scoping Reflection: Partial Reflection}
% ---------------------------------------------------------------------------
\todo{Do we only focus on behavioral reflection here?}
...
\begin{description}
	...
	\todo{Mention the C introducted limitiations in \P when optimizing message sends}
...
\Jikes itself is a research oriented \Java-based \VM framwork that provides signification infrastructure to facilitate \VM building.
...
In a direct metacircular \VM building process (see \figref{background-metacircular-bootstra}) it is inevitable to directly manage native-code.
...
Typically \VM frameworks focus on abstract at the code-level which should yield simpler code and thus help reducing development efforts.
...
\Quicktalk \cite{Ball86a} follows a similar path as we do with Waterfall presented in \secref{validation-waterfall}.
