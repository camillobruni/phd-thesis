...
An example of this is the previous code excerpt where we access the class of an object.
...
\noindent These four categories summarize the general properties of reflection.
In the following text we use the term "\emph{reflection}" or "\emph{dynamic reflection}" as a short form of unanticipated behavioral \emph{and} structural reflection both for performing intercession and introspection.
In the course of this dissertation we will introduce an additional category to distinguish between reflection that happens at language-side accessible by the developer and reflection that happens inside the \VM.

From now on we use a circular arrow (\includegraphics[height=10pt]{reflection-arrow-black}) to symbolize dynamic reflection in a figure.
...
%Although structural reflection can pose an overhead it does not directly influence the evaluation.
...
\item[Time:] Of course the most obvious axis is time itself.
	...
	This implies that the use of reflection changes over the course of evaluation.
...
% ---------------------------------------------------------------------------
\subsection{Reflection in \VMs and Language Runtimes}
\seclabel{background-vm-reflection}
% ---------------------------------------------------------------------------

\lt{I found section 2.1.2 hard to follow. I get the general idea, but the subsection is sometimes too vague, and other times too specific to easily follow. A separate issue is the slightly subjective nature of Figure 2.1; I think it might be worth being clearer that no such classification can ever be precise, and that it is best thought of as a way of understanding (rather than formally classifying) VMs.}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=\imagescale]{vm-reflection-evolution}
	\figlabel{vm-reflection-evolution}
	\caption[High-level Language Reflection Evolution]{
		Evolution of reflection in high-level language runtimes starting from a non-reflective language in a) and ending in a self-aware system f) where reflection transcends the \VM-language barrier.
		Circular arrows denote the use of reflection.}
\end{figure}
...
\item \textbf{Self-aware \VM:}
	...
	Unsurprisingly we find that these \VMs are built metacircularly, they are written in a subset or the same language they support.
	In the following \secref{background-open-vms} we will discuss in more details metacircular \VMs.
\end{enumerate}

\noindent From this overview of the evolution of reflection in high-level languages and their \VMs we see that there certain language runtimes that provide a form of \VM-level reflection.
...
% ===========================================================================
\section{High-level Languages and \VMs}
...
% ---------------------------------------------------------------------------
\todo{this should lead to high-level low-level programming in some sense}
...
\begin{figure}[h]
	\centering
	\vspace{1mm}
	\includegraphics[scale=\imagescale]{vm-metacircular-building-process}
	\vspace{-5mm}
...
\begin{figure}[h]
	\centering
	\vspace{2mm}
	\includegraphics[scale=\imagescale]{vm-metacircular-bootstrap}
	\caption[Metacircular \VM Bootstrap Types]{
	The indirect bootstrap on the left uses generated sources (\texttt{.c}) to compile a new \VM' with a C compiler (\texttt{CC}).
	The direct bootstrap on the right directly creates a new binary without the use of an intermediate low-level language.}
	\vspace{-5mm}
...
\urlfootnote{\LuaJIT}{https://github.com/jmckaskill/luaffi/} for instance is a very efficient \Lua implementation with a tracing \JIT.
\LuaJIT is interesting in terms of \FFI implementations as it directly inlines \FFI callouts into the \JIT compiled code.
