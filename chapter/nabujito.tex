...
This guarantees that the \VM stays as long as possible in the JIT context (see \secref{val-jit-interaction}). Additionally this enables new performance optimizations that for instance are hard to achieve with standard compliant C code.
...
From the in-production experience of \NB, the \B-based \FFI (see \secref{val-nb-performance}), we know that these costs amortized, especially for long-term applications.
...
However, we have shown in the performance evaluation for \NB, the \B-based \FFI, in \secref{val-nb-performance} that even a rather high one-time overhead is quickly amortized.
...
In particular \NB, the \B-based \FFI presented in \secref{val-ffi}, is used for interfacing with C libraries (\ttt{dlsym}). 

\paragraph{Primitives in \ST.}
As already partially explained, whenever a method is compiled with the \ttt{primitive} pragma as shown in \secref{val-vm-interaction} a flag is set on the \ttt{CompiledMethod}. 
...
\WF uses \B's mechanism for replacing primitive methods with customized versions that are nativized dynamically as described in \secref{val-benzo}.
...
\VM extensions are only justified in the presence of strong performance requirements (see \secref{val-related}).
...
In section \secref{val-wf-performance} we show how \WF, the \B based approach for generating primitives on the fly, outperforms the reflective solutions for primitives instrumentation. 
