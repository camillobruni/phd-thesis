...
\end{enumerate}
%
The example in Listing \lstref{basic-native-code} uses \Bs generator to create and install the native code which would trigger a low-level interrupt. Behind the scenes \B adds some more information to the code as the already mentioned platform marker. 
...
Waterfall uses \Bs mechanism for replacing primitive methods with customized versions that are nativized dynamically as described in Section~\secref{benzo}.
...
\reve{The fact that you used such a trivial method in the benchmark for Nabujito (the results of which are shown in Table 3) makes me wonder what's going on there. Could it be that the performance of Nabujito on a more realistic method would be so bad that you chose not to include any results? I don't think that's the case, but I think the paper should address this concern, either by including a less trivial benchmark, or by explaining why it does not matter that the benchmark that was used is so trivial.}
\cb{need to address this clearly again here!}
